...
def get_args():
    ...
    in_args.add_argument("-t", "--time",
                         help='''+/- time around a strand event, default is 10 seconds''',
                         type=int,
                         required=True,
                         default=10,
                         )
    ...
    out_args.add_argument("-g", "--generate-plot",
                          help="If set, do not generate density plot",
                          action="store_false",
                          default=True,
                          )
    out_args.add_argument("-A",
                         help="Column A time distributions as CSV",
                         type=full_path,
                         default='col_A.csv',
                         required=False,
                         )
    out_args.add_argument("-B",
                         help="Column B time distributions as CSV",
                         type=full_path,
                         default='col_B.csv',
                         required=False,
                         )
    out_args.add_argument("-C",
                         help="Column C time distributions as CSV",
                         type=full_path,
                         default='col_C.csv',
                         required=False,
                         )
    out_args.add_argument("-D",
                         help="Column D time distributions as CSV",
                         type=full_path,
                         default='col_D.csv',
                         required=False,
                         )
    out_args.add_argument("-E",
                          help="Column E time distributions as CSV",
                          type=full_path,
                          default="col_E.csv",
                          required=False,
                           )
    out_args.add_argument("-F",
                         help="Column F time distributions as CSV",
                         type=full_path,
                         default='col_F.csv',
                         required=False,
                         )
    out_args.add_argument("-o", "--out",
                          help='''Specify the output filename for the plot, defaults to \'classification_count.pdf\'.
                          Filetype must be one of [.eps, .ps, .tex, .pdf, .jpeg, .tiff, .png, .bmp, .svg, .wmf]''',
                          type=validate_file,
